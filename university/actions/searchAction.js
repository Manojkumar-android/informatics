...
export const getFacetsSearch = (data) => {

    return fetch(`${url}/university/getFacetsSearch`, {
        method: 'POST',
        headers: {
            Accept: "application/json",
            "Content-Type": "application/json",
        ...
        .then(response => {
            ...
            if (!response.ok) {
                // If the response status is not within the range 200-299
                return response.json().then(data => {
                    throw new Error(data.message || 'Failed to getFacetsSearch'); // Throw error with the message from response body
                });
            }
            return response.json(); // Parse JSON here
        })
